...
<body class="bg-[#7F64CE] min-h-screen flex items-center justify-center px-4 py-8 font-[Poppins]">
    <div class="w-full rounded-[30px] overflow-hidden shadow-2xl bg-[#F6F4FE]" style="max-width: 1000px;">
        <div class="grid grid-cols-1 lg:grid-cols-2 min-h-[700px]">
            <div class="relative flex items-start justify-center px-8 pt-10 pb-6 lg:px-16 bg-[#F6F4FE] overflow-hidden">
                ...
                <div class="w-full max-w-md relative z-10 animate-fadeIn">
                    <div class="mb-10 text-left" style="min-height: 120px;">
                        ...
                        <p class="mt-6 font-bold text-[#7F64CE] leading-tight"style="transform: translate(40px, -20px); font-size: 25px;"> Create Account</p>
                        <p style="transform: translate(40px, -15px); color: #4B5563; font-size: 14px;">Please enter your information to create your account.</p>
                    ...
                    <form method="POST" action="{{ route('register') }}" id="registerForm" class="space-y-3">
                        @csrf
                        <div>
                            <label for="name"class="block text-sm font-medium text-gray-700 mb-2"style="transform: translateX(40px);">Full Name</label>
                            <input id="name"  type="text" name="name" value="{{ old('name') }}"required autofocus autocomplete="name"style="width: 85%; display: block; margin: 0 auto;"class="rounded-2xl border @error('name') border-red-400 @else border-gray-200 @enderror bg-white px-5 py-3 text-gray-700 shadow-sm focus:border-[#C498F2]  focus:ring-2 focus:ring-[#C498F2]/40 focus:ring-offset-0 focus:outline-none transition">
                            @error('name')
                        ...
                        <div>
                            ...
                            <input id="username" type="text"  name="username" value="{{ old('username') }}"   required  autocomplete="username" style="width: 85%; display: block; margin: 0 auto;"class="rounded-2xl border @error('username') border-red-400 @else border-gray-200 @enderror bg-white px-5 py-3 text-gray-700 shadow-sm focus:border-[#C498F2]  focus:ring-2 focus:ring-[#C498F2]/40 focus:ring-offset-0 focus:outline-none transition">
                            @error('username')
                                <p class="mt-2 text-sm text-red-500" style="width: 85%; margin: 0 auto;">{{ $message }}</p>
                            @enderror
                        </div>
                        ...
                        <div>
                            ...
                            <input id="email" type="email" name="email" value="{{ old('email') }}" required autocomplete="email"  style="width: 85%; display: block; margin: 0 auto;" class="rounded-2xl border @error('email') border-red-400 @else border-gray-200 @enderror bg-white px-5 py-3 text-gray-700 shadow-sm focus:border-[#C498F2]  focus:ring-2 focus:ring-[#C498F2]/40 focus:ring-offset-0 focus:outline-none transition">
                            @error('email')
                                <p class="mt-2 text-sm text-red-500" style="width: 85%; margin: 0 auto;">{{ $message }}</p>
                            @enderror
                        </div>
                        ...
                        <div>
                            ...
                            <div class="relative" style="width: 85%; margin: 0 auto;">
                                <input id="password"type="password" name="password" required autocomplete="new-password" style="width: 100%;" class="block rounded-2xl border @error('password') border-red-400 @else border-gray-200 @enderror bg-white px-5 py-3 pr-12 text-gray-700 shadow-sm focus:border-[#C498F2]  focus:ring-2 focus:ring-[#C498F2]/40 focus:ring-offset-0 focus:outline-none transition">

                                <button type="button" onclick="togglePassword('password')" style="position: absolute; right: 15px; top: 50%; transform: translateY(-50%); color: #6B7280;">
